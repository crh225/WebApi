            ...
            IEnumerable<AggregateExpression> expressions = null) where T : DynamicTypeWrapper
        {
            ...
            if (expressions != null && expressions.Any())
            {
                foreach (var field in expressions)
                {
                    ...
                    {
                        var primitiveType = EdmLibHelpers.GetClrType(field.TypeReference, model, assembliesResolver);
            ...
            if (propertyNodes != null && propertyNodes.Any())
            {
                foreach (var field in propertyNodes)
                {
                    ...
                    {
                        var complexProp = GetResultType<DynamicTypeWrapper>(model, assembliesResolver, field.ChildTransformations);
