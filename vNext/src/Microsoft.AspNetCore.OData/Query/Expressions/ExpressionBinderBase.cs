...
namespace Microsoft.AspNetCore.OData.Query.Expressions
{
    internal abstract class ExpressionBinderBase
    {
        ...
        protected AssembliesResolver _assembliesResolver;

        protected ExpressionBinderBase(IEdmModel model, AssembliesResolver assembliesResolver, ODataQuerySettings querySettings)
        ...
        protected Expression CreateBinaryExpression(BinaryOperatorKind binaryOperator, Expression left, Expression right, bool liftToNull)
        {
            ...
            var leftUnderlyingTypeInfo = leftUnderlyingType.GetTypeInfo();
            Type rightUnderlyingType = Nullable.GetUnderlyingType(right.Type) ?? right.Type;
            var rightUnderlyingTypeInfo = rightUnderlyingType.GetTypeInfo();

            // Convert to integers unless Enum type is required
            if ((leftUnderlyingTypeInfo.IsEnum || rightUnderlyingTypeInfo.IsEnum) && binaryOperator != BinaryOperatorKind.Has)
            {
                Type enumType = leftUnderlyingTypeInfo.IsEnum ? leftUnderlyingType : rightUnderlyingType;
        ...
        protected Expression ConvertNonStandardPrimitives(Expression source)
        {
            ...
            Type conversionType = EdmLibHelpers.IsNonstandardEdmPrimitive(
                source.Type, 
                _assembliesResolver,
                out isNonstandardEdmPrimitive);
...
//#endif
        ...
        protected static bool IsNullable(Type t)
        {
            ...
            if (!ti.IsValueType || (ti.IsGenericType && t.GetGenericTypeDefinition() == typeof(Nullable<>)))
            {
                return true;
            }
